...
# data = np.load('output_data_single_leg.npz', allow_pickle=True)
# opt_leg = data['leg_opt']
data2 = np.load('database_single_leg.npz', allow_pickle=True)
database = data2['database']
dv = database[0, 1][0:3]
t_leg = database[0, 1][3]
opt_leg = Leg(dv, t_leg)
# EL PROBLEMA CON GUARDAR EN LA DATABASE [dv, t_leg] en vez de guardar directamente la leg s es que luego no puedes
#plotear la trayectoria a menos que vuelvas a integrar, ya que está guardada en s.
...
ax.plot(opt_leg.trajr[:, 0], opt_leg.trajr[:, 1], opt_leg.trajr[:, 2])
start_point = [opt_leg.trajr[0, 0], opt_leg.trajr[0, 1], opt_leg.trajr[0, 2]]
end_point = [opt_leg.trajr[-1, 0], opt_leg.trajr[-1, 1], opt_leg.trajr[-1, 2]]
ax.plot(start_point[0], start_point[1], start_point[2], 'g.', label='start point')
