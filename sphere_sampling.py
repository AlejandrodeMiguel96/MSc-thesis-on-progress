...
# picking points from surface of a sphere: https://www.bogotobogo.com/Algorithms/uniform_distribution_sphere.php
# picking points from surface of a sphere: https://www.bogotobogo.com/python/python_matplotlib.php#Point_distribution
# picking points from surface of a sphere: http://corysimon.github.io/articles/uniformdistn-on-sphere/
...
def sample_sphere(dvmin, dvmax, n):
    """
    Samples n_per_sphere random points uniformly from a sphere of radius dvmax.
...
def remove_unfeasible(p, treshold):
    ...
    :param p: vector containing the random uniform sampled points from a sphere
...
def dv_sampling(dvmin, dvmax, n):
    ...
    """
    ndim = 3
    n -= 1  # to account for the later addition of the no-impulse case [0,0,0]
    dv = np.array([])
    iterator = 1
    while len(dv) != n:
        if len(dv) < n:
...
def w_sampling(w_t_max, n):
    """
    Samples 3 random points uniformly from a sphere of radius w_t_max.
...
def q_sampling(n):
    """
    Samples n random points uniformly from the surface of a 4 dimension sphere or radius 1.
    :param n: number of desired sampled points.
    :return: p [n x dimension of sphere] vector containing the sampled points.
    """
