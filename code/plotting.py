...
start_point = legs_opt[0].rr_chaser_LVLH
for s in legs_opt:
    if len(s.rr_chaser_LVLH) == 0:
        break
    else:
        ax.plot(s.rr_chaser_LVLH[:, 0], s.rr_chaser_LVLH[:, 1], s.rr_chaser_LVLH[:, 2])
        end_point = [s.rr_chaser_LVLH[-1, 0], s.rr_chaser_LVLH[-1, 1], s.rr_chaser_LVLH[-1, 2]]
...
# start_point = opt_leg[0].rr_target
# for s in opt_leg:
#     if len(s.rr_chaser_LVLH) == 0:
#         break
#     else:
#         ax1.plot(s.rr_target[:, 0], s.rr_target[:, 1], s.rr_target[:, 2])
#         end_point = [s.rr_target[-1, 0], s.rr_target[-1, 1], s.rr_target[-1, 2]]
...
# start_point = opt_leg[0].rr_sun
# for s in opt_leg:
#     if len(s.rr_chaser_LVLH) == 0:
#         break
#     else:
#         ax2.plot(s.rr_sun[:, 0], s.rr_sun[:, 1], s.rr_sun[:, 2])
#         end_point = [s.rr_sun[-1, 0], s.rr_sun[-1, 1], s.rr_sun[-1, 2]]
...
# fig2 = plt.figure()
# ax3_1 = fig2.add_subplot(111)
# t = np.linspace(0, s.t_leg, n_steps)
# ax3_1.plot(t, opt_leg[0].w_vec[:, 0], label='wx')
# ax3_1.plot(t, opt_leg[0].w_vec[:, 1], label='wy')
# ax3_1.plot(t, opt_leg[0].w_vec[:, 2], label='wz')
# ax3_1.set_title("Target's angular velocity during the legs")
# ax3_1.set_xlabel('time')
# ax3_1.set_ylabel('w_vec')
# plt.legend()
# plt.grid()
#
# fig4 = plt.figure()
...
for s in legs_opt:
    if len(s.rr_chaser_LVLH) == 0:
        ...
        for x in s.rr_chaser_LVLH:
            r.append(np.linalg.norm(x[0:3]))
        for x in s.r_obs_chaser_vec_LVLH:
            r_obs.append(np.linalg.norm(x))
        for x in s.r_useful_vec_lvlh:
